...
* oTao数据翻译处理API
*/
if(!defined('TRANSLATOR_ENGINE')) define('TRANSLATOR_ENGINE','google_free');
class baseTranslateAPI{
	...
	var $translator_engine;
	var $translator_desc_engine;
	var $translator_prop_engine;
	var $translator_cn_engine;
	...
	protected function translator(){
		if(!$this->translator){
			...
			if($this->account_config)$this->translator->set_account_config($this->account_config);
	...
	protected function translator_prop(){
		if(!$this->translator_prop){
			...
			if($this->account_config)$this->translator_prop->set_account_config($this->account_config);
	...
	public function translate_default($text,$lang,$default_lang=null){
		if(!$this->translator_cn){
			...
			if($this->account_config)$this->translator_cn->set_account_config($this->account_config);
			
		}
		if(!$default_lang) $default_lang=$this->default_lang;
	...
	public function item_get($data){
			...
			if($data['item']['desc_short']) $t_array[]=$data['item']['desc_short'];
			...
			if($data['item']['property_alias'])
				$d_arr_alias=explode(';',$data['item']['property_alias']);//属性别名
			else
				$d_arr_alias=array();
			...
			if(count($t_array_t) == count($t_array)){
				foreach($t_array as $k=>$v)$t_array[$k]= str_replace('###short###','',$v);
				foreach($t_array_t as $k=>$v)if(!$v)$t_array_t[$k]=$t_array[$k];
				$t_array = array_combine($t_array,$t_array_t);
				
				
				$data['item']['title'] = $t_array[$data['item']['title']];
				if($data['item']['desc_short']) $data['item']['desc_short'] = $t_array[$data['item']['desc_short']];
			...
			if($data['item']['props_name']){
				...
				foreach($d_arr as $val){
					$arrtemp=explode(':',$val);
					if(!is_numeric($arrtemp[2])) $t_array[]=$arrtemp[2];
					if(!is_numeric($arrtemp[3])) $t_array[]=$arrtemp[3];
			...
			if($d_arr){
				foreach($d_arr as $dk=>$val){
					$val=explode(':',$val);
					if(!is_numeric($val[3]))$val[3]=$t_array[$val[3]];
					if($this->prop_lang!=$this->lang){
						if(!is_numeric($val[2]))$val[2]=$t_arrayp[$val[2]];
					}else{
						
						if(!is_numeric($val[2]))$val[2]=$t_array[$val[2]];
					}
			...
			if($data['item']['skus']['sku']){
				...
				foreach($t_array as $k=>$v)$t_array[$k]=str_replace(':','：',$v);//&#58;
				foreach($t_arrayp as $k=>$v)$t_arrayp[$k]=str_replace(':','：',$v);
		...
		if($data['item']['title'] && $data['item']['title']!=$this->data['item']['title']) $data['translate_status']='ok';
		else $data['translate_status']='error';
		return $data;
	}
	...
	public function item_search($data){
			...
			foreach($t_array_t as $tk=>$tv)
				if($t_array[$tk]&&!$tv)$translate_error++;
			
			$product_status=true;
			if($translate_error>count($t_array)/2)$product_status=false;//有一半的翻译不出来则不更新缓存
		...
		if($data['items']['item'][0]['title'] && $data['items']['item'][0]['title']!=$this->data['items']['item'][0]['title']) $data['translate_status']='ok';
		else $data['translate_status']='error';
	...
	*/
	...
	public function item_password($data){
		return $data;
	}
}
