...
class Request 
{
	static public function execute($url, $request_data = array(), $detect_encode = false) 
	{
		static $status;
		...
		if(!empty($urls['query'])){
			...
			if(!empty($query['route']))$route = str_replace('/','-',$query['route']);
		}
		if(isset($_GET['debug'])){
	...
	static protected function curl($url, $data = array()) 
	{
		$ch = curl_init();
