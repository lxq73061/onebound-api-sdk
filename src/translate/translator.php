...
{
	private $registry;
	static private $cn_codes = array('ZH', 'CN', 'ZH-CN', 'ZH_CN');
	static private $cn_code = 'ZH-CN';
	static private $en_codes = array('EN', 'EN-GB');
	static private $en_code = 'EN';
	static private $channels = array('google_free', 'google', 'baidu_free', 'baidu', 'baidu_old', 'microsoft', 'youdao_free', 'youdao');
	private $channel = 'google_free';
	private $google_account;
	private $google_domain = 'com';
	...
	static private $ignores = array('XXS', 'XS', 'S', 'M', 'L', 'XL', 'XXL', 'XXXL', 'XXXXL', 'XXXXXL', '2XL', '3XL', '4XL', '5XL');
	private $text;
	private $target;
	private $source;
	static private $filter_keywords = array('【天猫超市】', '包邮', '★', '^@^', '◤', '◢');
	...
	public function set_account_config($config){
		// $config['otao_translate_channel'] = 'google'; //使用google翻译
		...
		// 		),
		// );
		if(!empty($config['otao_translate_channel'])) $this->setChannel($config['otao_translate_channel']);
	...
	static public function getCnCodes() 
	{
		return self::$cn_codes;
	}
	static public function getCnCode() 
	{
		return self::$cn_code;
	}
	static public function getEnCodes() 
	{
		return self::$en_codes;
	}
	static public function getEnCode() 
	{
		return self::$en_code;
	}
...
// var_dump($data);
	...
	static public function replaceChinesePunctuation($text) 
	{
		$parttern = array('，', '。', '、', '“', '”', '‘', '’', '？', '；', '：', '【', '】', '『', '』', '《', '》', '！', '．', '＆', '－', '＇', '　', "\t", "\r\n", "\r", "\n");
		$replacement = array(',', '.', ',', '"', '"', '\'', '\'', '?', ';', ':', '[', ']', '{', '}', '<', '>', '!', '.', '&', '-', '\'', ' ', ' ', ' ', ' ', ' ');
