...
class ObApiHelper{
	...
	*/
	...
	public function short_url($url){
		...
		if(count($hosts)>2)$urls['host']=$hosts[1].'.'.$hosts[2];
		...
		if(in_array($urls['host'],$short_domains)){
			...
			if(strpos($url2,'://')!==false) return $url2;
		}
		return $url;
	}
	public function debug(){
		...
		echo '<fieldset><legend>api_data:</legend>';
			...
			echo '<strong>本地缓存用量</strong>:'.setupSize($curBytes).'，共：'.setupSize($totalBytes).'<br>';
		...
		if($api_data){
			...
			echo '<strong>本地缓存已经</strong>:'.(time()-$api_data['l_secache_time']).'秒<br>';
	...
	public function log($log,$log_file){
