...
class item_base{
	...
	*/
	function convert($api_name,$data,$value){
	...
	function convert_value(&$array,$key,$value){
		if(is_array($key)){
			foreach($key as $k)  $this->convert_value($array,$k,$value);
		}else{
	...
	function convert_item_get(&$data,$value){
		...
		if(is_array($data['item']['priceRange'])){
			foreach($data['item']['priceRange'] as  $k=>$v )$this->convert_value($data['item']['priceRange'][$k],1,$value);
		}
	}
}