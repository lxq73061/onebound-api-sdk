...
"CREATE TABLE IF NOT EXISTS `{prefix}_item_search` (
  ...
  PRIMARY KEY (`item_search_id`)
		...
		foreach ($this->client->apis as $key => $value) {
			if($value=='translate')continue;
			if($value!==$this->client->api_type)continue;
			foreach($dbs as $db){
	...
	public function cache_from_db(){
		...
		if($this->client->api_name=='item_search'){
			//return false;
			$keyword = trim($this->client->api_params['q']);
			$cat = $this->client->api_params['cat'];
			$page = $this->client->api_params['page'];
			//if($this->api_params['sort'])
			//排序
			//分类
			if(strpos($keyword,'_')===0)$keyword=substr($keyword,4);
			$item_total = 0;
			$where = 'WHERE 1';
			$wheres=array();
			if($keyword) {
			...
			if($item_total>20){
				$limit = ' LIMIT 0,40';
				if($page>1)$limit = ' LIMIT '.($page*40-40).','.($page*40).'';
...
// AND q=? 
		...
		if($this->client->api_name=='item_get'){
			...
			if($item){
				foreach($item as $k=>$v){
					if(strpos($v,'{')===0||strpos($v,'[')===0)$item[$k]=json_decode($v,true);
				}
				$seller = $this->cache_db_dbh->getRow('SELECT * FROM '.$table_prefix.'seller WHERE user_num_id=?',$item['seller_id']);
				if(!$seller)$seller=array();
				foreach($seller as $k=>$v){
					if(strpos($v,'{')===0||strpos($v,'[')===0)$seller[$k]=json_decode($v,true);
				}
		...
		if($this->client->api_name=='item_search'){
			...
			if($this->client->api_data['items']['item'][0]['title']&&1){
				foreach($this->client->api_data['items']['item'] as $k=>$v){
					...
					if(!$this->client->api_data['items']['cid']&&$this->api_params['cat'])$this->client->api_data['items']['cid']=$this->client->api_params['cat'];
					if($this->client->api_data['items']['cid'])$item['cid']=$this->client->api_data['items']['cid'];
...
$json = $this->client->api_data;//&&!$json['cache']
	...
	if($json['item']['title']){
		...
		foreach($item_key as $key){
			...
			if($key=='seller_info')$item['seller_id']=$json['item'][$key]['user_num_id'];
		}
	...
	*/
	public function secache(){
		if(!$this->secache) $this->secache=array();
		...
		if(empty($this->secache[$k])){
			if(SECACHE_SIZE==0){
				...
				$path = $this->secache_path.'/'.$this->client->api_type;
	...
	*/
	public function secache_clear($file=null){
		if($file){
			...
			$file = str_replace('\\','',$file);
			$file = str_replace(':','',$file);
			if(file_exists($this->secache_path.$file)) unlink($this->secache_path.$file);
			
		}else{
			$list = glob($this->secache_path.'*.php');
			foreach($list as $k=>$v) unlink($v);
		}
	...
	*/
	public function secache_list(){
		...
		$list = glob($this->secache_path.'*.php');
		foreach($list as $k=>$v)$list[$k] = str_replace($this->secache_path,'',$v);
		return $list;
